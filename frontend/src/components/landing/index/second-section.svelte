<div class="min-h-screen grid bg-gradient-to-b from-darkRed to-black p-8 h-fit">
	<div class="h-full w-full mx-auto flex flex-col gap-4 justify-center max-w-7xl">
		<!-- The Upper Big Section -->
		<div class="h-full w-full flex flex-col rounded-xl">
			<!-- Top Section -->
			<div class="relative -bottom-1 flex flex-row justify-between items-center">
				<h1 class="h1 relative -top-1">
					<img src="/svgs/logo.svg" alt="logo" class="h-20 w-20 inline" /> luceforge studio
				</h1>

				<!--  -->
				<div class="relative bg-lightRed p-4 rounded-t-xl pl-6">
					<div
						style="background: radial-gradient(140% 170% at left -25% top -70%, transparent 99%, #FF5A5F )"
						class="absolute top-0 right-full translate-x-0.5 translate-y-0.5 h-full w-20"
					/>
					<!--  -->
					<button class="btn variant-filled-secondary font-bold uppercase"
						><img
							src="https://api.iconify.design/hugeicons:target-03.svg"
							alt="target icon"
							class="h-6 w-6 mr-2"
						/> Join The Waitlist</button
					>
				</div>
			</div>

			<!-- Bottom Section -->
			<div
				class="flex-1 flex flex-row justify-between gap-8 w-full bg-lightRed rounded-tl-xl rounded-b-xl pr-16"
			>
				<!-- Text Side -->
				<div class="flex flex-col h-full justify-between p-8">
					<h2 class="text-black">MATCHMAKING</h2>

					<h1 class="h1 lg:text-6xl text-white uppercase">
						Meet with folks <br /> ready to hit <br /> the ground running
					</h1>
				</div>

				<!-- Globe Side -->
				<div class="flex">
					<img src="/svgs/globe.svg" alt="globe" class="w-80 h-80 m-auto" />
				</div>
			</div>
		</div>

		<!-- The Lower Compartments -->
		<div class="h-fit w-full flex flex-row gap-4">
			<!-- First -->
			<a
				href="/"
				class="group w-full h-full pt-24 relative flex items-end bg-lightRed text-black rounded-xl p-6"
			>
				<!-- Link Button -->
				<button
					class="aspect-square bg-black invert font-bold uppercase rounded-md absolute top-4 right-4"
				>
					<img src="/svgs/arrow-outward.svg" alt="link icon" class="h-12 w-12" />
				</button>
				<!--  -->
				<h1 class="h1 leading-none"><span class="text-7xl">1850</span> People ready to roll</h1>
				<!--  -->
			</a>
			<!-- Second -->
			<a
				href="/"
				class="group w-full h-full pt-24 relative flex items-end bg-lightRed text-black rounded-xl p-6"
			>
				<!-- Link Button -->
				<button
					class="aspect-square bg-black invert font-bold uppercase rounded-md absolute top-4 right-4"
				>
					<img src="/svgs/arrow-outward.svg" alt="link icon" class="h-12 w-12" />
				</button>
				<!--  -->
				<h1 class="h1 leading-none">Rolling features</h1>
				<!--  -->
			</a>
			<!-- Third -->
			<div class="w-full h-full pt-24 flex flex-col items-end bg-white text-black rounded-xl p-6">
				<h1 class="h1 leading-none">JOIN the buddy network</h1>
				<button
					class="w-full text-xl mt-2 btn bg-black hover:scale-110 transition-all text-white font-bold"
					><img
						src="https://api.iconify.design/fa6-solid:yin-yang.svg?color=%23ffffff"
						alt="yin-yang icon"
						class="h-6 w-6 mr-2"
					/>
					LET'S GO
					<img
						src="https://api.iconify.design/fa6-solid:yin-yang.svg?color=%23ffffff"
						alt="yin-yang icon"
						class="h-6 w-6 mr-2"
					/></button
				>
			</div>
		</div>
	</div>
</div>
